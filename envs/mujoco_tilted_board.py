...
class MujocoRigidState:
    ...
    step: int = 0
    time: float = 0.0
    integration_state: np.ndarray | None = None
    integration_spec: int | None = None
    ...
    def copy(self) -> "MujocoRigidState":
        return MujocoRigidState(
            ...
            integration_state=None if self.integration_state is None else self.integration_state.copy(),
...
class MujocoRigidTiltPushEnv:
    ...
    def set_state(self, state: MujocoRigidState) -> None:
        ...
        if state.integration_state is not None:
            self._mujoco.mj_setState(self.model, self.data, state.integration_state, state.integration_spec)
        else:
            # Legacy snapshots do not have solver memory. Never inherit it from
            # the previous candidate evaluated by this cached simulator.
        ...
        self._mujoco.mj_forward(self.model, self.data)
        self.data.qacc_warmstart[:] = warmstart
        self._stabilize_ball_speed()
        self._sync_state()
    ...
    def _sync_state(self) -> None:
        ...
        integration_spec = int(self._mujoco.mjtState.mjSTATE_INTEGRATION)
        integration_state = np.empty(self._mujoco.mj_stateSize(self.model, integration_spec), dtype=np.float64)
        self._mujoco.mj_getState(self.model, self.data, integration_state, integration_spec)
